...
class _ChannelListViewState extends State<ChannelListView> {
  ...
  ChannelListController get _channelListController => widget.channelListController ?? _defaultController;
  ...
  Widget _buildListView(BuildContext context, List<Channel?>? channels) {
    ...
    channels = channels
        ?.map((channel) =>
            channel?.state?.channelState?.messages != null && channel?.state?.channelState?.messages?.length != 0
                ? channel!
                : null)
        .toList();
  ...
  Shimmer _buildLoadingItem(BuildContext context) {
    ...
    if (widget.crossAxisCount > 1) {
      ...
      return Shimmer.fromColors(
        baseColor: chatThemeData.colorTheme.disabled,
        highlightColor: chatThemeData.colorTheme.white,
        child: ListTile(
          leading: Container(
